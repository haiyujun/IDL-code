...
localmin0=-20.334049
localmin1=-5.7398819
for m = -10, 10 do begin
for n = -10, 10 do begin
brk=0
y[0,0]=-(32.42+0.01*m)*1d6;x0(m)
y[0,1]=-(5.47+0.01*n)*1d6;z0(m)
...
;    ]
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    if y[i+1,0] LT y[i,0] then begin
        brk=1
        break
    endif
endfor
if brk EQ 1 then begin
    print,'m=',m,' n=',n,' break'
    continue
endif
;print,'part'
...
for i = 1, step-1 do begin
    if (y[i,1] GT y[i-1,1]) and (y[i,1] GT y[i+1,1]) then begin
        ...
        i_upMm[tmp]=i
        ;print,'x=',x_upMm[tmp],' z=',z_upMm[tmp],' kx=',y[i,2],' kz=',y[i,3],' i=',i
        tmp=tmp+1
    endif
endfor

if abs(x_upMm[1]-x_upMm[0]+17.05) LE abs(localmin1-localmin0+17.05) then begin
    localmin0=x_upMm[0]
    localmin1=x_upMm[1]
    if x_upMm[2] EQ 0 then print,'findnew m=',m,' n=',n
endif
...
;im=image(sqrt(br^2+bz^2),RGB_TABLE=ct,position=[0.1,0.2,0.9,1]);,max_value=6,min_value=2)
